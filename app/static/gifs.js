(function () {
  ...
  async function refreshQueue() {
    try {
      ...
      queueHasActivity = Boolean((data.running || []).length || (data.queued || []).length);
      updateQueue(data);
    } catch (e) {
  ...
  function initLogs() {
    ...
    const logsTab = byId('tab-logs');
    logsTab?.addEventListener('shown.bs.tab', () => {
      if (!document.hidden) startStream();
    });
    logsTab?.addEventListener('hidden.bs.tab', stopStream);
    if (logsTab?.classList.contains('active') && !document.hidden) startStream();
  }

  function activeWorkspaceTab() {
    return document.querySelector('[data-tab-hash].active')?.getAttribute('data-tab-hash') || 'new';
  }

  async function refreshWorkspace() {
    clearTimeout(workspaceRefreshTimer);
    if (document.hidden) {
      workspaceRefreshTimer = setTimeout(refreshWorkspace, 5000);
      return;
    }
    await refreshQueue();
    const activeTab = activeWorkspaceTab();
    if (activeTab === 'completed' || Date.now() - lastCompletedRefresh >= 30000) {
      await refreshCompleted();
    }
    const delay = queueHasActivity || activeTab === 'queue' ? 1000 : 5000;
    workspaceRefreshTimer = setTimeout(refreshWorkspace, delay);
  }
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    document.addEventListener('visibilitychange', () => {
      ...
      if (activeWorkspaceTab() === 'logs') startStream();
      refreshWorkspace();
    });
  });
}());
