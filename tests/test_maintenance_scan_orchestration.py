...
def test_scan_cache_recovers_last_known_good_backup(monkeypatch, tmp_path):
    ...
    scan = {
        ...
        "items": [],
        "settings": {"excluded_folders": {"trailers", "trailer"}},
    }
...
def test_scan_cache_is_not_restored_or_actionable_after_library_mount_changes(monkeypatch, tmp_path):
    ...
    allowed, error = maintenance_scan_store.action_allowed("duplicates", scan["id"])
    assert allowed is False
...
def test_full_scan_plan_expands_each_area_into_recorded_operation_stages():
    assert orchestrator._step_plan(["duplicates", "video_previews_quality", "posters"]) == [
        ...
        lambda scan_id: (
            {
                "scan": {
                    ...
                    "progress_percent": 100 if cancelled["value"] else 25,
                }
            },
            None,
        ),
...
def test_poster_analysis_is_non_mutating_and_page_scoped(monkeypatch, tmp_path):
    ...
    def dimensions(path):
        return (
            {"width": 1920, "height": 1080, "landscape": True}
            if "background" in os.path.basename(path)
            else {"width": 1000, "height": 1500, "landscape": False}
        )
    ...
    apply_run, error = poster_maintenance.start_poster_apply(plan["id"], synchronous=True, lib_root=str(library))
    assert error is None
