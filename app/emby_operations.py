...
def _fingerprint(settings):
    value = "|".join((str(settings.get("emby_url") or "").strip().rstrip("/"), str(settings.get("emby_api_key") or "")))
...
def _clean_text(value, settings, limit=500):
    ...
    for private in [
        settings.get("emby_url"),
        *(m.get("local_prefix") for m in settings.get("emby_path_mappings") or [] if isinstance(m, dict)),
    ]:
...
def _task_text(raw):
    return " ".join(
        str(raw.get(key) or "") for key in ("Name", "name", "Key", "key", "Description", "description")
    ).casefold()
...
def _public_task(raw, settings, *, controllable=False):
    ...
    task = {
        ...
        "triggers": [
            _trigger(item) for item in (raw.get("Triggers") or raw.get("triggers") or []) if isinstance(item, dict)
        ],
...
def load_tasks(settings=None, *, opener=None, force=False, now=None):
    ...
    if result.get("status") != "success":
        ...
        return _base(
            status,
            result.get("message") or "Emby scheduled tasks are unavailable",
            settings,
    ...
    if not isinstance(data, list) or any(not isinstance(item, dict) for item in data):
        return _base(
            "unavailable",
            "Emby returned an invalid scheduled-task response",
            settings,
            checked_at=result.get("checked_at"),
        )
    matches = [item for item in data if _is_thumbnail_task(item)]
    match_id = str(
        (matches[0] if len(matches) == 1 else {}).get("Id") or (matches[0] if len(matches) == 1 else {}).get("id") or ""
    )
    tasks = [
        _public_task(
            item, settings, controllable=bool(match_id and str(item.get("Id") or item.get("id") or "") == match_id)
        )
        for item in data
    ]
    payload = _base(
        ...
        failed_count=sum(
            1 for task in tasks if (task.get("last_result") or {}).get("status", "").casefold() in {"failed", "aborted"}
        ),
    )
    with _cache_lock:
...
def _control(task_id, action, settings=None, *, opener=None):
    ...
    if task.get("id") != inventory.get("thumbnail_task_id"):
        return {
            "status": "failed",
            "message": "vid2gif can only control Emby thumbnail extraction",
            "task": task,
            "result": None,
        }, "forbidden"
    ...
    if action == "start" and state in {"running", "cancelling"}:
        return {
            "status": "failed",
            "message": "Thumbnail extraction is already active",
            "task": task,
            "result": None,
        }, "conflict"
    if action == "start" and state != "idle":
        return {
            "status": "failed",
            "message": "Thumbnail extraction is not ready to start",
            "task": task,
            "result": None,
        }, "conflict"
    if action == "cancel" and state != "running":
        return {
            "status": "failed",
            "message": "Thumbnail extraction is not running",
            "task": task,
            "result": None,
        }, "conflict"
    task_id_path = urllib.parse.quote(str(task.get("id") or ""), safe="")
    path = (
        f"/ScheduledTasks/Running/{task_id_path}"
        if action == "start"
        else f"/ScheduledTasks/Running/{task_id_path}/Delete"
    )
...
def load_activity(settings=None, *, opener=None, limit=ACTIVITY_PUBLIC_LIMIT):
    ...
    if not settings.get("emby_url") or not settings.get("emby_api_key"):
        return {
            ...
            "checked_at": utc_iso(),
            "entries": [],
            "message": "Configure Emby to view task activity",
        }
    try:
    ...
    if result.get("status") != "success":
        ...
        return {
            ...
            "checked_at": result.get("checked_at"),
            "entries": [],
            "message": _clean_text(result.get("message"), settings),
        }
    if not isinstance(data, dict) or not isinstance(data.get("Items"), list):
        return {
            ...
            "checked_at": result.get("checked_at"),
            "entries": [],
            "message": "Emby returned an invalid activity response",
        }
    inventory = load_tasks(settings, opener=opener)
    terms = {
        str(value).casefold()
        for task in inventory.get("tasks") or []
        for value in (task.get("name"), task.get("key"))
        if value
    }
    entries = []
    ...
    return {
        ...
        "checked_at": result.get("checked_at"),
        "entries": entries,
        "message": f"Showing {len(entries)} recent task-related activity entries",
    }
