...
def test_worker_failure_leaves_existing_output_and_cleans_tmp_dir(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        jobs,
        "optimize_gif",
        lambda *args, **kwargs: (_ for _ in ()).throw(AssertionError("optimizer should not run after ffmpeg failure")),
    )
    monkeypatch.setattr(
        jobs,
        "record_successful_job",
        lambda job: (_ for _ in ()).throw(AssertionError("estimate history should not update after failure")),
    )
    t = threading.Thread(target=jobs.worker)
    t.start()
    jobs.job_queue.put(job_id)
